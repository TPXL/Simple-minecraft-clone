...
public class GLImage {
    ...
    public GLImage(String imgName)
    {
        ...
        if (makeGLImage(img,true,false)) {
			//GL_OBJ_Reader.msg("GLImage(String): loaded " + imgName + ", width=" + w + " height=" + h);
		}
    }
    ...
    public GLImage(String imgName, boolean flipYaxis, boolean convertPow2)
    {
        ...
        if (makeGLImage(img,flipYaxis,convertPow2)) {
			GL_OBJ_Reader.msg("GLImage(String,bool,bool): loaded " + imgName + ", width=" + w + " height=" + h);
		}
	}
    ...
    public GLImage(byte[] bytes) {
        ...
        if (makeGLImage(img,true,false)) {
          GL_OBJ_Reader.msg("GLImage(byte[]): loaded image from bytes[" + bytes.length + "]");
		}
		else {
		  System.out.println("GLImage(byte[]): could not create Image from bytes[" + bytes.length + "]");
		}
    }
    ...
    public GLImage(byte[] bytes, boolean flipYaxis, boolean convertPow2) {
        BufferedImage img = makeBufferedImage(bytes);
        if (makeGLImage(img,flipYaxis,convertPow2)) {
          GL_OBJ_Reader.msg("GLImage(byte[],bool,bool): loaded image from bytes[" + bytes.length + "]");
		}
		else {
			System.out.println("GLImage(byte[],bool,bool): could not create Image from bytes[" + bytes.length + "]");
		}
    }
    ...
    public BufferedImage makeBufferedImage(byte[] imageFileContents) {
    	...
    	try {
    	  ...
    	  System.out.println("GLImage.makeBufferedImage(): " + ioe);
    	}  
        return bi;
    }
    ...
    public BufferedImage loadJavaImage(String imgName) {
    	...
    	try {
    		tmpi = ImageIO.read(GL_OBJ_Reader.getInputStream(imgName));
    	}
    	catch (Exception e) {
    		System.out.println("GLImage.loadJavaImage() exception: FAILED TO LOAD IMAGE " + e);
    	}
    	return tmpi;
	}
    ...
    public static ByteBuffer convertImagePixelsARGB(int[] jpixels, int imgw, int imgh, boolean flipVertically) {
    	...
    	ByteBuffer bb = GLMaterialLib.allocBytes(jpixels.length * 4);  // 4 bytes per pixel
	...
	public static void savePixelsToPNG(int[] pixels, int width, int height, String imageFilename, boolean flipY) {
		if (pixels != null && imageFilename != null) {
			...
			try {
				...
				System.out.println("GLImage.savePixelsToPNG(" +imageFilename+ "): exception " + e);
			}
		}
	}
